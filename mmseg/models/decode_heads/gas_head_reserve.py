...
class GasHead(BaseDecodeHead):
    ...
    def __init__(self,
        ...
        super().__init__(
            ...
            act_cfg=act_cfg,
            **kwargs)
        self.i_head = BaseGASHead(in_channels, channels, norm_cfg, act_cfg)
        self.d_head = BaseGASHead(
            in_channels,
            in_channels // 4,
            norm_cfg,
        )
    ...
    def loss_by_feat(self, seg_logits: Tuple[Tensor],
        ...
        loss['loss_sem_i'] = self.loss_decode[0](i_logit, sem_label)
        loss['loss_bd'] = self.loss_decode[1](d_logit, bd_label)
        # filler = torch.ones_like(sem_label) * self.ignore_index
        # sem_bd_label = torch.where(
        #     torch.sigmoid(d_logit[:, 0, :, :]) > 0.8, sem_label, filler)
        # loss['loss_sem_bd'] = self.loss_decode[2](i_logit, sem_bd_label)
